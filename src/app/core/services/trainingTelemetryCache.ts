...
export async function clearTrainingTelemetryCache(): Promise<void> {
  ...
  const db = await openCacheDb();
  if (!db) return;
  const tx = db.transaction([JOBS_STORE, EVENTS_STORE, METRIC_BATCHES_STORE, META_STORE], "readwrite");
...
export async function pruneTrainingTelemetryCache(): Promise<void> {
  ...
  const [jobRows, eventRows, metricBatchRows] = await Promise.all([
  ...
  for (const candidate of trimCandidates) {
    ...
    else trimMetricBatchKeys.push(candidate.cacheKey);
  }
  ...
  if (jobKeysToDelete.length > 0 || eventKeysToDelete.length > 0 || metricBatchKeysToDelete.length > 0) {
    const tx = db.transaction([JOBS_STORE, EVENTS_STORE, METRIC_BATCHES_STORE], "readwrite");
    const jobStore = tx.objectStore(JOBS_STORE);
    const eventStore = tx.objectStore(EVENTS_STORE);
